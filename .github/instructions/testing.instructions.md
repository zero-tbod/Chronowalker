...
dotnet test -c Debug --filter "FullyQualifiedName~MainViewModelTests"

# Run a single test
dotnet test -c Debug --filter "FullyQualifiedName~MainViewModelTests.LoadItemsAsync_OnSuccess_PopulatesItems"

# Run all tests in a namespace (e.g., all ViewModel tests)
dotnet test -c Debug --filter "FullyQualifiedName~Tests.ViewModels"

# Run tests in a subfolder namespace (e.g., only Settings ViewModels)
dotnet test -c Debug --filter "FullyQualifiedName~Tests.ViewModels.Settings"
...
```powershell
dotnet test -c Debug --verbosity normal
```
...
4. **Run tests** — `dotnet test -c Debug` (from the platform-neutral core test project folder) and ensure all pass.
