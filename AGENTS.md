...
This is a **WinUI 3** desktop application built on the **Windows App SDK**. It uses self-contained, unpackaged deployment and supports x86, x64, and ARM64 architectures.
...
| Packaging | Unpackaged and self-contained (`<WindowsPackageType>None</WindowsPackageType>`) |
...
13. **Launch the built executable** -- See [Build, Run & Deploy](#build-run--deploy) below.
...
This is a self-contained, unpackaged WinUI 3 app. Pass both `-c` (Configuration) and `-p:Platform=` when building the app project.
...
- The .NET SDK version targeted by the project is required for development. The published app carries its .NET and Windows App SDK dependencies.
...
After building, launch the executable directly from the architecture-specific output folder. No package registration or Developer Mode is required.

### Run from the CLI

- Quick smoke tests: `dotnet run -c Debug -p:Platform=$Platform` from the
  project folder.
- Launch a build after setting `$Rid = $Platform.ToLowerInvariant()`: `Start-Process .\bin\$Platform\Debug\<TargetFramework>\win-$Rid\<ProjectName>.exe`.
...
```powershell
# The core tests are platform-neutral.
dotnet test .\<ProjectName>.Core.Tests\<ProjectName>.Core.Tests.csproj -c Debug
```
